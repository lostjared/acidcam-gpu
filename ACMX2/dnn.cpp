    ...
    TimedOutput benchmarkBackend(cv::dnn::Net &net, const cv::Mat &blob,
        ...
        measured.milliseconds =
            std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - start).count() / timedRuns;
        return measured;
    }
...
PPHS::PPHS(const std::string &modelPath, int backendId, int targetId)
    : impl_(std::make_unique<Impl>(modelPath, backendId, targetId)) {}

PPHS::~PPHS() = default;

cv::Mat PPHS::preprocess(const cv::Mat &image) {
    return impl_->preprocessImage(image);
}

cv::Mat PPHS::infer(const cv::Mat &image) {
    if (image.empty())
        return {};
    const cv::Mat input = impl_->preprocessImage(image);
    const cv::Mat output =
        selectBackendAndForward(impl_->model, impl_->backend, input,
                                impl_->inputName, impl_->outputName);
    return impl_->postprocessOutput(output);
}

cv::Mat PPHS::postprocess(const cv::Mat &outputBlob) {
    return impl_->postprocessOutput(outputBlob);
}
