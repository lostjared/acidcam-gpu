...
namespace {
    ...
    QHash<int, QString> default_slot_names() {
        return {{0, QStringLiteral("square_size")}, {1, QStringLiteral("alpha_value")}, {2, QStringLiteral("alpha_r")}, {3, QStringLiteral("alpha_g")}, {4, QStringLiteral("alpha_b")}, {5, QStringLiteral("value_alpha_r")}, {6, QStringLiteral("value_alpha_g")}, {7, QStringLiteral("value_alpha_b")}, {8, QStringLiteral("index_value")}, {9, QStringLiteral("restore_black")}, {10, QStringLiteral("seed")}, {11, QStringLiteral("value1")}, {12, QStringLiteral("iChannelTime")}, {13, QStringLiteral("time_speed")}, {14, QStringLiteral("blendAmt")}, {15, QStringLiteral("uDistortion")}, {16, QStringLiteral("uRotateSpeed")}, {17, QStringLiteral("uWarpSpeed")}, {18, QStringLiteral("uRandRate")}, {19, QStringLiteral("uPhaseRate")}, {20, QStringLiteral("slider1")}, {21, QStringLiteral("slider2")}, {22, QStringLiteral("slider3")}, {23, QStringLiteral("slider4")}, {24, QStringLiteral("frequency")}, {25, QStringLiteral("strength")}, {26, QStringLiteral("random_seed")}};
    }
    ...
    bool read_existing_metadata(const QString &path, QHash<QString, UniformMetadata> &metadata, QString &error) {
        if (!QFileInfo::exists(path))
            return true;

        QFile file(path);
        if (!file.open(QIODevice::ReadOnly)) {
            error = QObject::tr("Cannot read existing manifest %1: %2").arg(path, file.errorString());
            return false;
        }
        QJsonParseError parseError;
        const QJsonDocument document = QJsonDocument::fromJson(file.readAll(), &parseError);
        if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
            error = QObject::tr("Cannot parse existing manifest %1 at offset %2: %3").arg(path).arg(parseError.offset).arg(parseError.errorString());
            return false;
        }

        const QJsonObject uniforms = document.object().value(QStringLiteral("custom_uniforms")).toObject();
        for (auto it = uniforms.constBegin(); it != uniforms.constEnd(); ++it) {
            ...
            metadata.insert(it.key(), {value.value(QStringLiteral("minimum")).toDouble(fallback.minimum), value.value(QStringLiteral("maximum")).toDouble(fallback.maximum), value.value(QStringLiteral("step")).toDouble(fallback.step), value.value(QStringLiteral("value")).toDouble(fallback.value)});
        }
        return true;
    }

    bool scan_slots(const QString &path, QHash<int, QString> &slotNames, QHash<QString, int> &nameSlots, QString &error) {
        QFile file(path);
        ...
        static const QRegularExpression aliasExpression(QStringLiteral("^\\s*#define\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+"
        ...
        for (const QString &line : lines) {
            ...
            if (slotNames.contains(slot) && slotNames.value(slot) != name) {
                error = QObject::tr("Custom-uniform slot %1 is both %2 and %3 (%4)").arg(slot).arg(slotNames.value(slot), name, path);
                return false;
            }
            if (nameSlots.contains(name) && nameSlots.value(name) != slot) {
                error = QObject::tr("Custom uniform %1 uses slots %2 and %3 (%4)").arg(name).arg(nameSlots.value(name)).arg(slot).arg(path);
                return false;
            }
...
namespace acmx2 {
    static bool create_acmxvk_source_manifest_impl(const QString &rootDirectory, const QString &requestedOutputPath, const QStringList *includedShaders, AcmxvkSourceManifestResult &result, QString &error) {
        result = {};
        error.clear();

        const QFileInfo rootInfo(rootDirectory);
        if (!rootInfo.exists() || !rootInfo.isDir()) {
            error = QObject::tr("Shader source directory does not exist: %1").arg(rootDirectory);
            return false;
        }
        const QString root = QDir(rootInfo.absoluteFilePath()).canonicalPath();
        const QString outputPath = requestedOutputPath.trimmed().isEmpty() ? QDir(root).filePath(QStringLiteral("library.json")) : QFileInfo(requestedOutputPath).absoluteFilePath();
        if (!QDir().mkpath(QFileInfo(outputPath).absolutePath())) {
            error = QObject::tr("Cannot create manifest directory: %1").arg(QFileInfo(outputPath).absolutePath());
            return false;
        }
        ...
        if (includedShaders) {
            for (QString entry : *includedShaders) {
                ...
                if (entry.isEmpty() || entry == QStringLiteral(".") || entry == QStringLiteral("..") || entry.startsWith(QStringLiteral("../")) || QDir::isAbsolutePath(entry)) {
                    error = QObject::tr("Invalid shader path in source manifest: %1").arg(entry);
                    return false;
                }
                if (includedFiles.contains(entry)) {
                    error = QObject::tr("Duplicate shader path in source manifest: %1").arg(entry);
                    return false;
                }
        ...
        while (iterator.hasNext()) {
            ...
            const int directoryPartCount = std::max(0, static_cast<int>(parts.size()) - 1);
            const bool underCompute = parts.mid(0, directoryPartCount).contains(QStringLiteral("compute"));
            const bool computeShader = underCompute && relative.endsWith(QStringLiteral(".comp"));
            const bool fragmentShader = !underCompute && relative.endsWith(QStringLiteral(".frag"));
        ...
        if (includedShaders) {
            for (const QString &entry : includedFiles) {
                if (!discoveredFiles.contains(entry)) {
                    error = QObject::tr("Manifest shader is missing or is not a supported "
        ...
        if (shaders.isEmpty()) {
            error = QObject::tr("No .frag or compute/*.comp sources found in %1").arg(root);
            return false;
        }
        ...
        for (const QString &shader : shaders) {
            ...
            if (outputNames.contains(compiled)) {
                error = QObject::tr("Case-insensitive duplicate shader output: %1.spv").arg(shader);
                return false;
            }
        ...
        for (int slot = 0; slot <= maximumSlot; ++slot) {
            if (!slotNames.contains(slot)) {
                error = QObject::tr("No custom-uniform name was found for slot %1").arg(slot);
                return false;
            }
        }
        ...
        if (!output.open(QIODevice::WriteOnly)) {
            error = QObject::tr("Cannot write %1: %2").arg(outputPath, output.errorString());
            return false;
        }
        const QByteArray json = QJsonDocument(manifest).toJson(QJsonDocument::Indented);
        if (output.write(json) != json.size() || !output.commit()) {
            error = QObject::tr("Cannot replace %1: %2").arg(outputPath, output.errorString());
            return false;
        }
    ...
    bool create_acmxvk_source_manifest(const QString &rootDirectory, const QString &outputPath, AcmxvkSourceManifestResult &result, QString &error) { return create_acmxvk_source_manifest_impl(rootDirectory, outputPath, nullptr, result, error); }

    bool create_acmxvk_source_manifest_for_shaders(const QString &rootDirectory, const QStringList &shaderFiles, const QString &outputPath, AcmxvkSourceManifestResult &result, QString &error) { return create_acmxvk_source_manifest_impl(rootDirectory, outputPath, &shaderFiles, result, error); }
} // namespace acmx2
