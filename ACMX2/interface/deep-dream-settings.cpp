...
namespace {
    class DreamChannelSpinBox final : public QSpinBox {
      public:
        explicit DreamChannelSpinBox(QWidget *parent = nullptr) : QSpinBox(parent) {}

      protected:
        QValidator::State validate(QString &text, int &position) const override {
            const QString value = text.trimmed();
            if (value.compare("A", Qt::CaseInsensitive) == 0 || value.compare("All", Qt::CaseInsensitive) == 0 || value.compare("All channels", Qt::CaseInsensitive) == 0) {
...
DeepDreamSettingsDialog::DeepDreamSettingsDialog(bool gpu_filter_enabled, QWidget *parent) : QDialog(parent), gpu_filter_available(gpu_filter_enabled) {
    ...
    layer_combo_box->addItems({"relu1_1", "relu1_2", "relu2_1", "relu2_2", "relu3_1", "relu3_2", "relu3_3", "relu4_1", "relu4_2", "relu4_3", "relu5_1", "relu5_2", "relu5_3"});
    ...
    fp16_check_box->setToolTip("Use half-precision model and working tensors to reduce CUDA memory "
    ...
    channel_spin_box->setToolTip("Enter a channel number, A, or All to select every channel.");
    ...
    gpu_filter_first_check_box = new QCheckBox("Run acidcam-gpu filters before Deep Dream", this);
    gpu_filter_first_check_box->setToolTip(gpu_filter_available ? "Transform the CUDA source with acidcam-gpu before LibTorch." : "Enable an acidcam-gpu filter chain first.");
    deep_original_check_box = new QCheckBox("Independent-frame preview (--deep-orig)", this);
    deep_original_check_box->setToolTip("Process each video frame independently without temporal feedback, "
    ...
    performance_layout->addRow("Maximum dimension:", maximum_dimension_spin_box);
    ...
    QPushButton *randomize_button = buttons->addButton("Randomize", QDialogButtonBox::ActionRole);
    randomize_button->setToolTip("Generate and immediately apply a random Deep Dream preset.");
    auto *layout = new QVBoxLayout(this);
    layout->addWidget(scroll_area, 1);
    layout->addWidget(buttons);

    connect(enable_check_box, &QCheckBox::toggled, this, [this](bool) { update_enabled_state(); });
    connect(native_size_check_box, &QCheckBox::toggled, this, [this](bool) { update_enabled_state(); });
    connect(deep_original_check_box, &QCheckBox::toggled, this, [this](bool) { update_enabled_state(); });
    connect(browse_model_button, &QPushButton::clicked, this, &DeepDreamSettingsDialog::browse_model);
    connect(buttons, &QDialogButtonBox::accepted, this, &DeepDreamSettingsDialog::accept_settings);
    connect(buttons->button(QDialogButtonBox::Apply), &QPushButton::clicked, this, &DeepDreamSettingsDialog::apply_settings);
    connect(randomize_button, &QPushButton::clicked, this, &DeepDreamSettingsDialog::randomize_settings);
...
DeepDreamConfiguration DeepDreamSettingsDialog::configuration() const {
    ...
    result.maximum_dimension = native_size_check_box->isChecked() ? 0 : maximum_dimension_spin_box->value();
...
void DeepDreamSettingsDialog::browse_model() {
    ...
    QString directory = settings.value("deep_dream/last_model_directory").toString();
    if (directory.isEmpty()) {
        directory = QFileInfo(model_file_edit->text()).absolutePath();
    }
    const QString filename = QFileDialog::getOpenFileName(this, "Select Deep Dream TorchScript Model", directory, "TorchScript Models (*.pt *.pth);;All Files (*)");
    if (filename.isEmpty()) {
        return;
    }
    model_file_edit->setText(filename);
    refresh_model_metadata(true);
    settings.setValue("deep_dream/last_model_directory", QFileInfo(filename).absolutePath());
}
...
void DeepDreamSettingsDialog::refresh_model_metadata(bool report_error) {
    ...
    if (!metadata_info.isFile()) {
        model_metadata_label->setText("No .pt.json sidecar found; enter a feature layer manually.");
        return;
    }
    constexpr qint64 MAXIMUM_METADATA_BYTES = 1024 * 1024;
    if (metadata_info.size() <= 0 || metadata_info.size() > MAXIMUM_METADATA_BYTES) {
    ...
    if (error.isEmpty()) {
        ...
        document = QJsonDocument::fromJson(metadata_file.readAll(), &parse_error);
    ...
    if (error.isEmpty()) {
        ...
        if (root.value("format").toString() != "acmxvk-deep-dream" || root.value("version").toInt(-1) != 1) {
            ...
            static const QRegularExpression name_pattern(QStringLiteral("^[A-Za-z0-9_.-]{1,64}$"));
            if (!name_pattern.match(architecture).hasMatch() || layer_array.isEmpty() || layer_array.size() > 256) {
                ...
                for (const QJsonValue value : layer_array) {
                    ...
                    if (!name_pattern.match(name).hasMatch() || layers.contains(name)) {
            ...
            if (error.isEmpty() && (!layers.contains(default_layer) || minimum_size < 1 || minimum_size > 4096)) {
    ...
    if (!error.isEmpty()) {
        ...
        if (report_error) {
            QMessageBox::warning(this, "Invalid Deep Dream Metadata", error + "\n\n" + metadata_filename);
        }
        return;
    }
    ...
    layer_combo_box->setCurrentText(layers.contains(selected_layer) ? selected_layer : default_layer);
    model_metadata_label->setText(QString("%1, %2 feature layers, minimum input %3 px").arg(architecture).arg(layers.size()).arg(minimum_size));
    model_metadata_label->setToolTip(metadata_filename);
}
...
bool DeepDreamSettingsDialog::validate_settings() {
    if (enable_check_box->isChecked()) {
        ...
        if (!model.isFile()) {
            QMessageBox::warning(this, "Deep Dream Model Required", "Select an existing TorchScript model file.");
            model_file_edit->setFocus();
            return false;
        }
        static const QRegularExpression layer_pattern(QStringLiteral("^[A-Za-z0-9_.-]+$"));
        if (!layer_pattern.match(layer_combo_box->currentText().trimmed()).hasMatch()) {
            QMessageBox::warning(this, "Invalid Deep Dream Layer", "Enter a named layer such as relu4_2 or a numeric layer index.");
            layer_combo_box->setFocus();
            return false;
        }
        if (gpu_filter_first_check_box->isChecked() && !gpu_filter_available) {
...
void DeepDreamSettingsDialog::randomize_settings() {
    if (!QFileInfo(model_file_edit->text().trimmed()).isFile()) {
        QMessageBox::warning(this, "Deep Dream Model Required", "Select a TorchScript model before randomizing.");
        model_file_edit->setFocus();
        return;
    }
    ...
    if (layer_combo_box->count() > 0) {
        layer_combo_box->setCurrentIndex(random->bounded(layer_combo_box->count()));
    }
    ...
    const double rotation_magnitude = 0.5 + random->generateDouble() * 2.5;
    rotation_spin_box->setValue(random->bounded(2) == 0 ? -rotation_magnitude : rotation_magnitude);
    static constexpr std::array<int, 4> RANDOM_DIMENSIONS = {256, 384, 512, 640};
    native_size_check_box->setChecked(false);
    maximum_dimension_spin_box->setValue(RANDOM_DIMENSIONS[static_cast<std::size_t>(random->bounded(static_cast<int>(RANDOM_DIMENSIONS.size())))]);
    apply_settings();
}
...
void DeepDreamSettingsDialog::load_ui_state() {
    QSettings settings("LostSideDead", "acmx2");
    enable_check_box->setChecked(settings.value("deep_dream/enabled", false).toBool());
    model_file_edit->setText(settings.value("deep_dream/model_file", QString()).toString());
    layer_combo_box->setCurrentText(settings.value("deep_dream/layer", "relu4_2").toString());
    iterations_spin_box->setValue(settings.value("deep_dream/iterations", 1).toInt());
    strength_spin_box->setValue(settings.value("deep_dream/strength", 0.05).toDouble());
    feedback_spin_box->setValue(settings.value("deep_dream/feedback", 0.9).toDouble());
    zoom_spin_box->setValue(settings.value("deep_dream/zoom", 1.01).toDouble());
    rotation_spin_box->setValue(settings.value("deep_dream/rotation", 0.1).toDouble());
    const int maximum_dimension = settings.value("deep_dream/maximum_dimension", 512).toInt();
    native_size_check_box->setChecked(maximum_dimension == 0);
    maximum_dimension_spin_box->setValue(maximum_dimension == 0 ? 512 : maximum_dimension);
    fp16_check_box->setChecked(settings.value("deep_dream/fp16", false).toBool());
    channel_spin_box->setValue(settings.value("deep_dream/channel", -1).toInt());
    octaves_spin_box->setValue(settings.value("deep_dream/octaves", 1).toInt());
    octave_scale_spin_box->setValue(settings.value("deep_dream/octave_scale", 1.4).toDouble());
    jitter_spin_box->setValue(settings.value("deep_dream/jitter", 0).toInt());
    smoothing_spin_box->setValue(settings.value("deep_dream/smoothing", 0).toInt());
    gpu_filter_first_check_box->setChecked(gpu_filter_available && settings.value("deep_dream/gpu_filter_first", false).toBool());
    deep_original_check_box->setChecked(settings.value("deep_dream/deep_original", false).toBool());
}
...
void DeepDreamSettingsDialog::save_ui_state() {
    ...
    settings.setValue("deep_dream/maximum_dimension", current.maximum_dimension);
    ...
    settings.setValue("deep_dream/gpu_filter_first", current.gpu_filter_first);
...
void DeepDreamSettingsDialog::update_enabled_state() {
    ...
    const std::array<QWidget *, 16> controls = {model_file_edit, browse_model_button, layer_combo_box, model_metadata_label, iterations_spin_box, strength_spin_box, feedback_spin_box, zoom_spin_box, rotation_spin_box, native_size_check_box, fp16_check_box, channel_spin_box, octaves_spin_box, octave_scale_spin_box, jitter_spin_box, smoothing_spin_box};
    for (QWidget *widget : controls) {
        widget->setEnabled(enabled);
    }
    maximum_dimension_spin_box->setEnabled(enabled && !native_size_check_box->isChecked());
    const bool temporal_feedback_enabled = enabled && !deep_original_check_box->isChecked();
