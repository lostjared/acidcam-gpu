...
building, playlist parsing, resource discovery, media utilities, asynchronous
camera capture, interface shared-memory access, and background snapshot
encoding use normal `.hpp`/`.cpp` pairs; only the tightly coupled render-window
implementation remains in ordered `.ipp` sections.
