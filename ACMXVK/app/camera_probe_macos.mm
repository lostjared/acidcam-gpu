...
namespace acmxvk {
    namespace {
        ...
        [[nodiscard]] std::string fourccName(OSType subtype) {
            char name[5]{static_cast<char>((subtype >> 24U) & 0xFFU), static_cast<char>((subtype >> 16U) & 0xFFU), static_cast<char>((subtype >> 8U) & 0xFFU), static_cast<char>(subtype & 0xFFU), '\0'};
            bool printable = true;
            for (int index = 0; index < 4; ++index) {
                const unsigned char character = static_cast<unsigned char>(name[index]);
                printable = printable && character >= 0x20U && character <= 0x7EU;
            }
            if (printable) {
                return name;
            }
...
#pragma clang diagnostic pop
            for (NSUInteger index = 0; index < [devices count]; ++index) {
                ...
                output << index << '\t' << stringValue([device localizedName]) << '\n';
            }
            if ([devices count] == 0U) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            NSArray<AVCaptureDevice *> *devices = [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo];
#pragma clang diagnostic pop
            if (device_index < 0 || device_index >= static_cast<int>([devices count])) {
                error << "acmxvk: camera device index " << device_index << " is unavailable; AVFoundation found " << [devices count] << " video device(s)\n";
                return false;
            }

            AVCaptureDevice *device = [devices objectAtIndex:static_cast<NSUInteger>(device_index)];
            ...
            for (AVCaptureDeviceFormat *device_format in [device formats]) {
                CMFormatDescriptionRef description = [device_format formatDescription];
                const CMVideoDimensions dimensions = CMVideoFormatDescriptionGetDimensions(description);
                const std::string format = fourccName(CMFormatDescriptionGetMediaSubType(description));
                const ModeKey key{format, dimensions.width, dimensions.height};
                CameraMode &mode = modes[key];
                mode.format = format;
                mode.width = dimensions.width;
                mode.height = dimensions.height;

                for (AVFrameRateRange *range in [device_format videoSupportedFrameRateRanges]) {
            ...
            if (modes.empty()) {
                error << "acmxvk: AVFoundation camera " << device_index << " exposes no capture formats\n";
                return false;
            }
            ...
            for (auto &[key, mode] : modes) {
                ...
                if (mode.format != current_format) {
                    ...
                    output << "\n  Format: " << current_format << " (AVFoundation)\n";
                }
                std::sort(mode.frame_rates.begin(), mode.frame_rates.end(), std::greater<double>());
