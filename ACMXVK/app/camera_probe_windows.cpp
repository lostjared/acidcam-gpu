...
namespace acmxvk {
    namespace {
        ...
        class ComScope {
          public:
            ComScope() : result(CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED)), should_uninitialize(SUCCEEDED(result)) {}
            ...
            [[nodiscard]] bool ready() const { return SUCCEEDED(result) || result == RPC_E_CHANGED_MODE; }
        ...
        [[nodiscard]] std::string wide_to_utf8(const wchar_t *value) {
            ...
            if (WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, value, source_length, result.data(), length, nullptr, nullptr) != length) {
                return {};
            }
            return result;
        }
        ...
        [[nodiscard]] std::string camera_name(IPropertyBag *property_bag) {
            ...
            for (const wchar_t *property : {L"FriendlyName", L"Description"}) {
                VARIANT value;
                VariantInit(&value);
                const HRESULT result = property_bag->Read(property, &value, nullptr);
                std::string name;
        ...
        [[nodiscard]] std::vector<CameraDevice> enumerate_devices() {
            ...
            if (FAILED(CoCreateInstance(CLSID_SystemDeviceEnum, nullptr, CLSCTX_INPROC_SERVER, IID_ICreateDevEnum, reinterpret_cast<void **>(&device_enumerator_raw)))) {
                return {};
            }
            ComPtr<ICreateDevEnum> device_enumerator(device_enumerator_raw);

            IEnumMoniker *enumerator_raw = nullptr;
            if (device_enumerator->CreateClassEnumerator(CLSID_VideoInputDeviceCategory, &enumerator_raw, 0) != S_OK || enumerator_raw == nullptr) {
                return {};
            }
            ...
            while (enumerator->Next(1, &moniker_raw, &fetched) == S_OK) {
                ...
                if (SUCCEEDED(moniker->BindToStorage(nullptr, nullptr, IID_IPropertyBag, reinterpret_cast<void **>(&property_bag_raw)))) {
        ...
        [[nodiscard]] ComPtr<IAMStreamConfig> stream_config(IMoniker *moniker) {
            IBaseFilter *filter_raw = nullptr;
            if (moniker == nullptr || FAILED(moniker->BindToObject(nullptr, nullptr, IID_IBaseFilter, reinterpret_cast<void **>(&filter_raw)))) {
                return {};
            }
            ...
            while (pins->Next(1, &pin_raw, &fetched) == S_OK) {
                ...
                if (SUCCEEDED(pin->QueryInterface(IID_IAMStreamConfig, reinterpret_cast<void **>(&config_raw))) && config_raw != nullptr) {
        ...
        [[nodiscard]] bool video_format(const AM_MEDIA_TYPE *media_type, int &width, int &height, double &frame_rate) {
            ...
            if (bitmap == nullptr || bitmap->biWidth <= 0 || bitmap->biHeight == 0) {
                return false;
            }
            width = bitmap->biWidth;
            height = std::abs(bitmap->biHeight);
            frame_rate = interval > 0 ? 10000000.0 / static_cast<double>(interval) : 0.0;
            return true;
        }
        ...
        void append_frame_rate(std::vector<double> &rates, double value) {
            ...
            if (std::none_of(rates.begin(), rates.end(), [value](double rate) { return std::abs(rate - value) < 0.05; })) {
    ...
    [[nodiscard]] bool probeCameraDevice(int device_index, std::ostream &output, std::ostream &error) {
        ...
        const auto device = std::find_if(devices.begin(), devices.end(), [device_index](const CameraDevice &entry) { return entry.index == device_index; });
        if (device == devices.end()) {
            error << "acmxvk: DirectShow camera " << device_index << " was not found\n";
            return false;
        }
        const ComPtr<IAMStreamConfig> config = stream_config(device->moniker.get());
        if (!config) {
            error << "acmxvk: camera " << device_index << " exposes no DirectShow capture configuration\n";
            return false;
        }

        int capability_count = 0;
        int capability_size = 0;
        if (FAILED(config->GetNumberOfCapabilities(&capability_count, &capability_size)) || capability_count <= 0 || capability_size <= 0) {
            error << "acmxvk: camera " << device_index << " exposes no DirectShow capture formats\n";
            return false;
        }

        using ResolutionMap = std::map<std::pair<int, int>, std::vector<double>>;
        std::map<std::string, ResolutionMap> formats;
        std::vector<BYTE> capability_buffer(static_cast<std::size_t>(capability_size));
        for (int index = 0; index < capability_count; ++index) {
            AM_MEDIA_TYPE *media_type = nullptr;
            if (FAILED(config->GetStreamCaps(index, &media_type, capability_buffer.data())) || media_type == nullptr) {
                continue;
            }
            const std::unique_ptr<AM_MEDIA_TYPE, decltype(&free_media_type)> media_type_guard(media_type, &free_media_type);
            int width = 0;
            int height = 0;
            ...
            std::vector<double> &rates = formats[subtype_name(media_type->subtype)][{width, height}];
            append_frame_rate(rates, frame_rate);
            if (capability_size >= static_cast<int>(sizeof(VIDEO_STREAM_CONFIG_CAPS))) {
                const auto *caps = reinterpret_cast<const VIDEO_STREAM_CONFIG_CAPS *>(capability_buffer.data());
                if (caps->MinFrameInterval > 0) {
                    append_frame_rate(rates, 10000000.0 / static_cast<double>(caps->MinFrameInterval));
                }
                if (caps->MaxFrameInterval > 0) {
                    append_frame_rate(rates, 10000000.0 / static_cast<double>(caps->MaxFrameInterval));
                }
            }
        }
        ...
        for (auto &[format, resolutions] : formats) {
            ...
            for (auto &[resolution, rates] : resolutions) {
                ...
                output << "    " << resolution.first << 'x' << resolution.second;
                bool first = true;
                for (const double rate : rates) {
                    output << (first ? " @ " : ", ") << std::fixed << std::setprecision(1) << rate << " fps";
                    first = false;
                }
                output << '\n';
            }
