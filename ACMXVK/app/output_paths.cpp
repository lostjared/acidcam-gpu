...
namespace acmxvk {
    fs::path output_frame_directory(const std::string &filename, std::string_view suffix) {
        ...
        const fs::path parent = output_path.has_parent_path() ? output_path.parent_path() : fs::path(".");
        const std::string name = output_path.filename().empty() ? std::string("output") : output_path.filename().string();
    ...
    void create_output_directory(const fs::path &directory) {
        ...
        if (error || !fs::is_directory(directory)) {
            throw std::runtime_error("unable to create PNG output directory: " + directory.string());
        }
    }
    ...
    fs::path snapshot_path(const fs::path &directory, std::uint32_t width, std::uint32_t height, std::uint64_t &counter, SnapshotFormat format, std::chrono::system_clock::time_point timestamp) {
        const std::time_t timestamp_time = std::chrono::system_clock::to_time_t(timestamp);
        std::tm local_time{};
#ifdef _WIN32
        ...
        while (true) {
            std::ostringstream filename;
            filename << "ACMXVK.Snapshot-" << std::put_time(&local_time, "%Y.%m.%d-%H.%M.%S") << '-' << width << 'x' << height << '-' << counter << snapshot_extension(format);
