...
namespace acmxvk {
    ...
    fs::path find_shader_path(const std::vector<fs::path> &available_shaders, const fs::path &library_directory, std::string name) {
        ...
        const auto match = std::find_if(available_shaders.begin(), available_shaders.end(), [&](const fs::path &shader) { return shader.filename() == requested.filename() || (!library_directory.empty() && shader.lexically_relative(library_directory) == requested); });
    ...
    std::vector<PlaylistNode> load_playlist(const fs::path &playlist_path, const std::vector<fs::path> &available_shaders, const fs::path &library_directory, std::ostream &warning_output) {
        ...
        if (!playlist_input) {
            throw std::runtime_error("unable to open playlist: " + playlist_path.string());
        }
        ...
        while (input::read_bounded_line(playlist_input, line, "shader playlist", line_number++)) {
            ...
            if (line.size() >= 2 && line.front() == '[' && line.back() == ']') {
                if (playlist.size() >= input::MAX_PLAYLIST_NODES) {
                    throw std::runtime_error("shader playlist contains too many nodes");
                }
                std::string node_name = trim_text(line.substr(1, line.size() - 2));
                input::validate_string(node_name, input::StringKind::DisplayText, "shader playlist node");
                playlist.push_back({std::move(node_name), {}});
                current_node = &playlist.back();
                continue;
            }
            if (line.front() == '[' || line.back() == ']') {
                throw std::runtime_error("malformed shader playlist node at line " + std::to_string(line_number - 1));
            }
            if (++entry_count > input::MAX_PLAYLIST_ENTRIES) {
                throw std::runtime_error("shader playlist contains too many entries");
            }
            input::validate_string(line, input::StringKind::Path, "shader playlist entry");

            const fs::path shader = find_shader_path(available_shaders, library_directory, line);
            if (shader.empty()) {
        ...
        playlist.erase(std::remove_if(playlist.begin(), playlist.end(), [](const PlaylistNode &node) { return node.shaders.empty(); }), playlist.end());
