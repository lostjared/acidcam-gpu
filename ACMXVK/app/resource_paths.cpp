...
namespace acmxvk {
    namespace {
        ...
        [[nodiscard]] fs::path executable_resource_directory() {
            ...
            for (;;) {
                ...
                if (length < buffer.size() - 1U) {
                    ...
                    return executable.parent_path().parent_path() / "share" / "acmxvk";
                }
        ...
        [[nodiscard]] fs::path resolve_resource(const Options &options, const fs::path &relative_path, const fs::path &installed_path, const fs::path &built_path) {
    ...
    fs::path find_resource(const Options &options, const fs::path &relative_path) {
        ...
        if (relative_text == ".." || relative_text.starts_with("../") || relative_text.find("/../") != std::string::npos) {
            return {};
        }
        for (const fs::path &directory : resource_directories(options)) {
            const fs::path candidate = (directory / normalized_relative).lexically_normal();
    ...
    fs::path hdr_transfer_shader_path(const Options &options, bool hlg, bool encode) {
        const std::string filename = std::string("hdr_") + (hlg ? "hlg_" : "pq_") + (encode ? "encode.frag.spv" : "decode.frag.spv");
        return resolve_resource(options, fs::path("shaders") / filename, fs::path(ACMXVK_INSTALL_HDR_TRANSFER_DIRECTORY) / filename, fs::path(ACMXVK_BUILD_HDR_TRANSFER_DIRECTORY) / filename);
    }

    fs::path hdr_preview_shader_path(const Options &options, bool hlg) {
        const std::string filename = std::string("hdr_preview_") + (hlg ? "hlg" : "pq") + ".frag.spv";
        return resolve_resource(options, fs::path("shaders") / filename, fs::path(ACMXVK_INSTALL_HDR_TRANSFER_DIRECTORY) / filename, fs::path(ACMXVK_BUILD_HDR_TRANSFER_DIRECTORY) / filename);
    }

    fs::path human_composite_shader_path(const Options &options) { return resolve_resource(options, "shaders/human_composite.frag.spv", ACMXVK_INSTALL_HUMAN_COMPOSITE_SHADER, ACMXVK_BUILD_HUMAN_COMPOSITE_SHADER); }
    ...
    fs::path crossfade_shader_path(const Options &options, std::size_t shader_index) {
        ...
        const std::string filename = std::string(CROSSFADE_NAMES[shader_index]) + ".frag.spv";
        const fs::path resource = find_resource(options, fs::path("shaders/xfade") / filename);
        if (!resource.empty()) {
            return resource;
        }
        const fs::path installed = fs::path(ACMXVK_INSTALL_CROSSFADE_DIRECTORY) / filename;
        if (fs::is_regular_file(installed)) {
            return installed;
        }
        const fs::path built = fs::path(ACMXVK_BUILD_CROSSFADE_DIRECTORY) / filename;
        if (fs::is_regular_file(built)) {
            return built;
        }
        throw std::runtime_error("crossfade shader was not found: " + filename);
    }

    fs::path model_vertex_shader_path(const Options &options) { return resolve_resource(options, "shaders/model.vert.spv", ACMXVK_INSTALL_MODEL_VERTEX_SHADER, ACMXVK_BUILD_MODEL_VERTEX_SHADER); }

    fs::path model_fragment_shader_path(const Options &options) { return resolve_resource(options, "shaders/model.frag.spv", ACMXVK_INSTALL_MODEL_FRAGMENT_SHADER, ACMXVK_BUILD_MODEL_FRAGMENT_SHADER); }

    fs::path default_model_path(const Options &options) { return resolve_resource(options, "models/cube.obj", ACMXVK_INSTALL_DEFAULT_MODEL, ACMXVK_BUILD_DEFAULT_MODEL); }

    fs::path overlay_font_path(const Options &options) { return resolve_resource(options, "data/font.ttf", ACMXVK_INSTALL_OVERLAY_FONT, ACMXVK_BUILD_OVERLAY_FONT); }
} // namespace acmxvk
