...
namespace acmxvk {
    [[nodiscard]] std::vector<fs::path> resource_directories(const Options &options);
    [[nodiscard]] fs::path find_resource(const Options &options, const fs::path &relative_path);
    [[nodiscard]] fs::path mxvk_shader_directory(const Options &options);
    [[nodiscard]] fs::path sprite_vertex_shader_path(const Options &options);
    ...
    [[nodiscard]] fs::path human_composite_shader_path(const Options &options);
    [[nodiscard]] fs::path crossfade_shader_path(const Options &options, std::size_t shader_index);
