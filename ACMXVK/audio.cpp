...
namespace acmxvk::audio {
      ...
      public:
        ...
        bool open(const AudioStreamConfig &config) {
            ...
            try {
                ...
                const auto selected = std::find(device_ids.begin(), device_ids.end(), input_device);
                if (selected == device_ids.end()) {
                    std::cerr << "acmxvk: audio input device " << input_device << " was not found\n";
                    return false;
                }

                const RtAudio::DeviceInfo input_info = stream.getDeviceInfo(input_device);
                input::validate_string(input_info.name, input::StringKind::DisplayText, "audio input device name");
                if (input_info.inputChannels == 0) {
                    std::cerr << "acmxvk: audio device " << input_device << " has no input channels\n";
                    return false;
                }

                input_channels = std::min(std::max(config.channels, 1U), input_info.inputChannels);
                pass_through = config.pass_through;
                pass_through_gain = std::clamp(config.pass_through_gain, 0.0F, 4.0F);
                recording_gain = std::clamp(config.recording_gain, 0.0F, 2.0F);
                output_channels = 0;
                sensitivity_value.store(std::clamp(config.sensitivity, 0.1F, 5.0F), std::memory_order_relaxed);
                resetMetrics();

                unsigned int selected_rate = 44100;
                if (!input_info.sampleRates.empty() && std::find(input_info.sampleRates.begin(), input_info.sampleRates.end(), selected_rate) == input_info.sampleRates.end()) {
                    selected_rate = 48000;
                    if (std::find(input_info.sampleRates.begin(), input_info.sampleRates.end(), selected_rate) == input_info.sampleRates.end()) {
                ...
                if (pass_through) {
                    output_device = config.output_device >= 0 ? static_cast<unsigned int>(config.output_device) : stream.getDefaultOutputDevice();
                    const auto output_selected = std::find(device_ids.begin(), device_ids.end(), output_device);
                    if (output_selected == device_ids.end()) {
                        std::cerr << "acmxvk: audio output device " << output_device << " was not found; live pass-through disabled\n";
                        pass_through = false;
                    } else {
                        const RtAudio::DeviceInfo output_info = stream.getDeviceInfo(output_device);
                        input::validate_string(output_info.name, input::StringKind::DisplayText, "audio output device name");
                ...
                const auto supports_rate = [](const std::vector<unsigned int> &rates, unsigned int rate) { return rates.empty() || std::find(rates.begin(), rates.end(), rate) != rates.end(); };
                if (pass_through && !supports_rate(output_sample_rates, selected_rate)) {
                    std::vector<unsigned int> candidates{44100, 48000};
                    candidates.insert(candidates.end(), input_info.sampleRates.begin(), input_info.sampleRates.end());
                    candidates.insert(candidates.end(), output_sample_rates.begin(), output_sample_rates.end());
                    const auto common_rate = std::find_if(candidates.begin(), candidates.end(), [&](unsigned int rate) { return supports_rate(input_info.sampleRates, rate) && supports_rate(output_sample_rates, rate); });
                ...
                stream.openStream(output_parameters_ptr, &input_parameters, RTAUDIO_FLOAT32, selected_rate, &buffer_frames, &Impl::audioCallback, this);
                stream.startStream();

                std::cout << "acmxvk: audio input " << input_device << ": " << input_info.name << " (" << selected_rate << " Hz, " << input_channels << " channel" << (input_channels == 1 ? "" : "s") << ")\n";
                if (pass_through) {
                    std::cout << "acmxvk: live audio pass-through " << output_device << ": " << output_name << " (" << output_channels << " channel" << (output_channels == 1 ? "" : "s") << ", gain " << pass_through_gain << ")\n";
                }
        ...
        [[nodiscard]] std::vector<float> spectrum() const {
            ...
            for (std::size_t index = 0; index < AudioEngine::FFT_SIZE; ++index) {
                const float hann = 0.5F * (1.0F - std::cos(2.0F * PI * static_cast<float>(index) / static_cast<float>(AudioEngine::FFT_SIZE - 1)));
                complex[2 * index] = spectrum_samples[front][index].load(std::memory_order_relaxed) * hann;
            }
            ...
            for (std::size_t index = 0; index < magnitudes.size(); ++index) {
                ...
                magnitudes[index] = std::sqrt(real * real + imaginary * imaginary) * INVERSE;
            }
            return magnitudes;
        }
        ...
        static int audioCallback(void *output_buffer, void *input_buffer, unsigned int frame_count, double, RtAudioStreamStatus status, void *user_data) { return static_cast<Impl *>(user_data)->processCallback(static_cast<float *>(output_buffer), static_cast<const float *>(input_buffer), frame_count, status); }
                ...
                if (samples == nullptr || !pass_through) {
                    ...
                    for (unsigned int frame = 0; frame < frame_count; ++frame) {
                        for (unsigned int channel = 0; channel < output_channels; ++channel) {
                            ...
                            output[frame * output_channels + channel] = std::clamp(samples[frame * input_channels + input_channel] * pass_through_gain, -1.0F, 1.0F);
                        }
                    }
                }
            }
        ...
        int processSamples(const float *samples, unsigned int frame_count) {
            ...
            const int spectrum_back = 1 - spectrum_front.load(std::memory_order_acquire);
            const float rate = static_cast<float>(std::max(sample_rate.load(std::memory_order_relaxed), 1U));
            ...
            if (capture_recording) {
                ...
                capture_recording = recording.load(std::memory_order_acquire);
            }
            ...
            for (unsigned int frame = 0; frame < frame_count; ++frame) {
                ...
                if (capture_recording) {
                    recorded_samples.push_back(std::clamp(mono * recording_gain, -1.0F, 1.0F));
                }
                ...
                if (frame < AudioEngine::FFT_SIZE) {
                    spectrum_samples[spectrum_back][frame].store(mono, std::memory_order_relaxed);
                }
                peak_value = std::max(peak_value, std::abs(mono));
                square_sum += mono * mono;
                if (frame > 0 && ((previous >= 0.0F && mono < 0.0F) || (previous < 0.0F && mono >= 0.0F))) {
                    ++crossings;
                }
                previous = mono;
            }
            if (capture_recording) {
                recorded_sample_count.fetch_add(frame_count, std::memory_order_release);
            }
            for (std::size_t frame = std::min<std::size_t>(frame_count, AudioEngine::FFT_SIZE); frame < AudioEngine::FFT_SIZE; ++frame) {
                spectrum_samples[spectrum_back][frame].store(0.0F, std::memory_order_relaxed);
            }
            spectrum_front.store(spectrum_back, std::memory_order_release);

            const float amplitude_value = amplitude_sum / static_cast<float>(frame_count * input_channels);
        ...
        bool startRecording() {
            if (!stream.isStreamOpen() || recording.load(std::memory_order_acquire)) {
                return false;
            }

            std::lock_guard<std::mutex> lock(recording_mutex);
            recorded_samples.clear();
            recording_sample_rate = std::max(sample_rate.load(std::memory_order_relaxed), 1U);
            recorded_sample_count.store(0, std::memory_order_relaxed);
            constexpr unsigned int RESERVE_SECONDS = 60;
            recorded_samples.reserve(static_cast<std::size_t>(recording_sample_rate) * RESERVE_SECONDS);
            recording.store(true, std::memory_order_release);
            std::cout << "acmxvk: live audio recording started (" << recording_sample_rate << " Hz, mono, gain " << recording_gain << ")\n";
            return true;
        }

        [[nodiscard]] double recordingTime() const {
            const unsigned int rate = std::max(recording_sample_rate, 1U);
            return static_cast<double>(recorded_sample_count.load(std::memory_order_acquire)) / static_cast<double>(rate);
        }
        ...
        [[nodiscard]] AudioRecording stopRecording() {
            ...
            if (!result.empty()) {
                std::cout << "acmxvk: live audio recording stopped (" << result.duration_seconds() << " seconds)\n";
            }
            recorded_samples.clear();
            return result;
        }
    ...
    AudioMetrics AudioEngine::metrics() const { return impl->metrics(); }

    unsigned int AudioEngine::sample_rate() const { return impl->sample_rate.load(std::memory_order_relaxed); }

    void AudioEngine::set_sensitivity(float sensitivity) { impl->sensitivity_value.store(std::clamp(sensitivity, 0.1F, 5.0F), std::memory_order_relaxed); }

    float AudioEngine::sensitivity() const { return impl->sensitivity_value.load(std::memory_order_relaxed); }

    std::vector<float> AudioEngine::spectrum() const { return impl->spectrum(); }

    void AudioEngine::process_samples(const float *samples, unsigned int frame_count, unsigned int channels, unsigned int sample_rate) {
    ...
    bool AudioEngine::start_recording() { return impl->startRecording(); }

    AudioRecording AudioEngine::stop_recording() { return impl->stopRecording(); }

    bool AudioEngine::is_recording() const { return impl->recording.load(std::memory_order_acquire); }

    double AudioEngine::recording_time() const { return impl->recordingTime(); }
    ...
    bool write_wav_file(const AudioRecording &recording, const std::string &filename) {
        ...
        const std::uint64_t data_size_64 = static_cast<std::uint64_t>(recording.samples.size()) * sizeof(std::int16_t);
        ...
        std::array<char, SAMPLE_BUFFER_SIZE * sizeof(std::int16_t)> sample_buffer{};
        std::size_t offset = 0;
        while (offset < recording.samples.size()) {
            const std::size_t sample_count = std::min(SAMPLE_BUFFER_SIZE, recording.samples.size() - offset);
            for (std::size_t index = 0; index < sample_count; ++index) {
                const float recorded_sample = recording.samples[offset + index];
                const float sample = std::isfinite(recorded_sample) ? std::clamp(recorded_sample, -1.0F, 1.0F) : 0.0F;
                const auto pcm_sample = static_cast<std::int16_t>(std::lround(sample * 32767.0F));
                const auto sample_bits = static_cast<std::uint16_t>(pcm_sample);
                sample_buffer[index * 2U] = static_cast<char>(sample_bits & 0xFFU);
                sample_buffer[index * 2U + 1U] = static_cast<char>((sample_bits >> 8U) & 0xFFU);
            }
            output.write(sample_buffer.data(), static_cast<std::streamsize>(sample_count * sizeof(std::int16_t)));
            if (!output) {
                return false;
            }
            offset += sample_count;
    ...
    void AudioEngine::reset() { impl->resetMetrics(); }

    void AudioEngine::list_devices() {
        try {
            RtAudio stream = makeRtAudio();
            const std::vector<unsigned int> device_ids = stream.getDeviceIds();
            std::cout << "acmxvk: found " << device_ids.size() << " audio device(s)\n";
            for (const unsigned int id : device_ids) {
                const RtAudio::DeviceInfo info = stream.getDeviceInfo(id);
                input::validate_string(info.name, input::StringKind::DisplayText, "audio device name");
            ...
            throw std::runtime_error(std::string("could not enumerate audio devices: ") + error.what());
        }
    }
