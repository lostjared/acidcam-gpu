...
namespace acmxvk::dream {
    ...
    struct Model::Impl {
        ...
        [[nodiscard]] TensorAscentResult optimize(
            const torch::Tensor &input,
            const GradientAscentOptions &options);
    };

    [[nodiscard]] TensorAscentResult Model::Impl::optimize(
        const torch::Tensor &input,
        const GradientAscentOptions &options) {
        const torch::Device device(torch::kCUDA, cuda_device);
        const torch::Tensor mean =
            normalization_tensor(metadata.input_mean, device, scalar_type);
        const torch::Tensor standard_deviation =
            normalization_tensor(metadata.input_std, device, scalar_type);
        ...
        const double minimum_scale = std::min(
            1.0, std::max(static_cast<double>(metadata.minimum_input_size) /
                              input_height,
                          static_cast<double>(metadata.minimum_input_size) /
                              input_width));
        for (int octave = options.octaves - 1; octave >= 0; --octave) {
            ...
            const std::int64_t height = std::max<std::int64_t>(
                metadata.minimum_input_size,
                static_cast<std::int64_t>(std::lround(input_height * scale)));
            const std::int64_t width = std::max<std::int64_t>(
                metadata.minimum_input_size,
                static_cast<std::int64_t>(std::lround(input_width * scale)));
        ...
        for (std::size_t octave_index = 0;
            ...
            for (int iteration = 0; iteration < options.iterations;
                ...
                const std::vector<torch::Tensor> outputs =
                    feature_outputs(module.forward({model_input}));
    ...
    [[nodiscard]] GradientAscentResult Model::apply_gradient_ascent(
        ...
        const torch::Tensor input =
            torch::from_blob(rgb_float.data, {rgb_float.rows, rgb_float.cols, 3},
                ...
                .contiguous();
        TensorAscentResult tensor_result =
            implementation->optimize(input, options);
    ...
    [[nodiscard]] GradientAscentResult Model::apply_gradient_ascent_cuda(
        const cv::cuda::GpuMat &rgba, cv::cuda::GpuMat &output,
        cv::cuda::Stream &stream, const GradientAscentOptions &options) {
        validate_gradient_options(options);
        if (rgba.empty() || rgba.type() != CV_8UC4 || rgba.cols < 1 ||
            rgba.rows < 1) {
            throw std::runtime_error(
                "Deep Dream CUDA input must be a non-empty RGBA8 image");
        }
        ...
        if (resize_scale < 1.0) {
            const cv::Size working_size(
                std::max(1, static_cast<int>(std::lround(rgba.cols * resize_scale))),
                std::max(1, static_cast<int>(std::lround(rgba.rows * resize_scale))));
            cv::cuda::resize(rgba, implementation->working_input_cuda,
                             working_size, 0.0, 0.0, cv::INTER_AREA, stream);
            working_rgba = &implementation->working_input_cuda;
        }
        if (working_rgba->cols <
                implementation->metadata.minimum_input_size ||
            working_rgba->rows <
                implementation->metadata.minimum_input_size) {
            throw std::runtime_error(
                "Deep Dream CUDA working image is smaller than the model minimum");
        }

        const cv::cuda::GpuMat *dream_source = working_rgba;
        if (options.feedback > 0.0F &&
            !implementation->previous_output_cuda.empty() &&
            implementation->previous_output_cuda.type() == CV_8UC4 &&
            implementation->previous_output_cuda.size() ==
                working_rgba->size()) {
            const cv::Point2f center(
                static_cast<float>(working_rgba->cols - 1) * 0.5F,
                static_cast<float>(working_rgba->rows - 1) * 0.5F);
            const cv::Mat transform = cv::getRotationMatrix2D(
                center, options.rotation_degrees, options.zoom);
            cv::cuda::warpAffine(
                implementation->previous_output_cuda,
                implementation->transformed_feedback_cuda, transform,
                working_rgba->size(), cv::INTER_LINEAR,
                cv::BORDER_REFLECT_101, cv::Scalar(), stream);
            cv::cuda::addWeighted(
                implementation->transformed_feedback_cuda, options.feedback,
                *working_rgba, 1.0F - options.feedback, 0.0,
                implementation->blended_input_cuda, -1, stream);
        ...
        const torch::Tensor source_rgba = torch::from_blob(
            rgba.data, {rgba.rows, rgba.cols, 4},
            {static_cast<std::int64_t>(rgba.step), 4, 1}, byte_options);
        const torch::Tensor working_rgba_tensor = torch::from_blob(
            dream_source->data,
            {dream_source->rows, dream_source->cols, 4},
            {static_cast<std::int64_t>(dream_source->step), 4, 1},
            byte_options);
        const torch::Tensor input =
            working_rgba_tensor.narrow(2, 0, 3)
                .permute({2, 0, 1})
                .unsqueeze(0)
                .to(implementation->scalar_type)
                .div(255.0F)
                .contiguous();

        TensorAscentResult tensor_result =
            implementation->optimize(input, options);
        const auto rgba_tensor = [](const torch::Tensor &rgb,
                                    const torch::Tensor &alpha) {
            const torch::Tensor rgb_bytes =
                rgb.squeeze(0)
                    .permute({1, 2, 0})
                    .mul(255.0F)
                    .round()
        ...
        const torch::Tensor working_output = rgba_tensor(
            tensor_result.output,
            working_rgba_tensor.narrow(2, 3, 1));
        implementation->previous_output_cuda.create(
            dream_source->rows, dream_source->cols, CV_8UC4);
        torch::cuda::synchronize(implementation->cuda_device);
        check_cuda(
            cudaMemcpy2D(implementation->previous_output_cuda.data,
                         implementation->previous_output_cuda.step,
                         working_output.data_ptr<std::uint8_t>(),
                         static_cast<std::size_t>(dream_source->cols) * 4U,
                         static_cast<std::size_t>(dream_source->cols) * 4U,
                         dream_source->rows, cudaMemcpyDeviceToDevice),
            "Deep Dream could not preserve its CUDA feedback frame");

        torch::Tensor final_rgb = tensor_result.output;
        if (dream_source->cols != rgba.cols ||
            dream_source->rows != rgba.rows) {
            final_rgb = torch::nn::functional::interpolate(
                final_rgb,
                torch::nn::functional::InterpolateFuncOptions()
                    .size(std::vector<std::int64_t>{rgba.rows, rgba.cols})
                    .mode(torch::kBilinear)
                    .align_corners(false));
        }
        const torch::Tensor final_output =
            rgba_tensor(final_rgb, source_rgba.narrow(2, 3, 1));
        torch::cuda::synchronize(implementation->cuda_device);
        output.create(rgba.rows, rgba.cols, CV_8UC4);
        check_cuda(
            cudaMemcpy2D(output.data, output.step,
                         final_output.data_ptr<std::uint8_t>(),
                         static_cast<std::size_t>(rgba.cols) * 4U,
                         static_cast<std::size_t>(rgba.cols) * 4U, rgba.rows,
                         cudaMemcpyDeviceToDevice),
            "Deep Dream could not publish its CUDA output frame");
