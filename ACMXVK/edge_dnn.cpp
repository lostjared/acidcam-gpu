...
namespace acmxvk::dnn {
    namespace {
        ...
        [[nodiscard]] cv::Mat runForward(cv::dnn::Net &net, const cv::Mat &blob, const cv::String &input_name, const cv::String &output_name) {
            if (input_name.empty()) {
                net.setInput(blob);
            } else {
        ...
        [[nodiscard]] cv::Mat selectBackendAndForward(cv::dnn::Net &net, BackendState &state, const cv::Mat &blob, const cv::String &input_name, const cv::String &output_name) {
            ...
            const bool fp16_available = backendAvailable(cv::dnn::DNN_BACKEND_CUDA, cv::dnn::DNN_TARGET_CUDA_FP16);
        ...
        [[nodiscard]] cv::Mat spatialPlane(const cv::Mat &output) {
            if (output.dims == 4 && output.size[0] == 1 && output.size[1] == 1 && output.type() == CV_32F) {
        ...
        void validateMapKeys(const cv::FileNode &node, const std::set<std::string> &allowed, std::string_view context) {
            if (!node.isMap()) {
                throw std::runtime_error(std::string(context) + " must be a YAML mapping");
            }
            for (auto iterator = node.begin(); iterator != node.end(); ++iterator) {
                const std::string name = (*iterator).name();
                if (!allowed.contains(name)) {
                    throw std::runtime_error(std::string(context) + " contains unsupported field '" + name + "'");
                }
            }
        }

        [[nodiscard]] std::string readString(const cv::FileNode &node, std::string_view context, bool allow_empty = false) {
            if (node.empty() || !node.isString()) {
                throw std::runtime_error(std::string(context) + " must be a string");
            }
            std::string value;
            node >> value;
            input::validate_string(value, input::StringKind::Path, context, allow_empty);
            return value;
        }

        [[nodiscard]] double readNumber(const cv::FileNode &node, double default_value, std::string_view context) {
            if (node.empty()) {
        ...
        [[nodiscard]] int readInteger(const cv::FileNode &node, int default_value, std::string_view context) {
        ...
        [[nodiscard]] bool readBoolean(const cv::FileNode &node, bool default_value, std::string_view context) {
            if (node.empty()) {
                return default_value;
            }
            if (node.isInt() || node.isReal()) {
        ...
        [[nodiscard]] std::string readYamlText(const std::filesystem::path &path) {
            input::validate_text_file(path, "ONNX YAML configuration");
            std::ifstream stream(path, std::ios::binary);
            if (!stream) {
                throw std::runtime_error("unable to open ONNX YAML configuration: " + path.string());
            }
            std::string text((std::istreambuf_iterator<char>(stream)), std::istreambuf_iterator<char>());
            if (!text.starts_with("%YAML")) {
                const std::size_t first_content = text.find_first_not_of(" \t\r\n");
                const bool has_document_marker = first_content != std::string::npos && text.compare(first_content, 3, "---") == 0;
        ...
        void validateTensorName(std::string_view name) {
            ...
            if (name.size() > 256U || !std::all_of(name.begin(), name.end(), [](char value) {
                    const auto character = static_cast<unsigned char>(value);
                    return std::isalnum(character) != 0 || value == '_' || value == ':' || value == '/' || value == '.' || value == '-';
                })) {
                throw std::runtime_error("model.input contains unsupported characters");
            }
        }

        [[nodiscard]] cv::Mat buildHardenedFloatAlpha(const cv::Mat &image, const cv::Mat &mask, float black_point, float white_point) {
            constexpr int MAXIMUM_WORK_DIMENSION = 512;
            const int image_max_dimension = std::max(image.cols, image.rows);
            const double work_scale = image_max_dimension > MAXIMUM_WORK_DIMENSION ? static_cast<double>(MAXIMUM_WORK_DIMENSION) / image_max_dimension : 1.0;
            const cv::Size work_size(std::max(1, cvRound(image.cols * work_scale)), std::max(1, cvRound(image.rows * work_scale)));
            ...
            if (open_size > 1) {
                const cv::Mat kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(open_size, open_size));
                cv::morphologyEx(binary, binary, cv::MORPH_OPEN, kernel);
            }
            if (close_size > 1) {
                const cv::Mat kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(close_size, close_size));
            ...
            cv::Mat stats;
            cv::Mat centroids;
            const int label_count = cv::connectedComponentsWithStats(binary, labels, stats, centroids, 8, CV_32S);
            if (label_count > 1) {
                int best_label = -1;
                int best_area = 0;
                ...
                const int minimum_area = (binary.cols * binary.rows) / 200;
                if (best_label > 0 && best_area >= minimum_area) {
                    cv::compare(labels, best_label, binary, cv::CMP_EQ);
                }
            }
            if (erode_size > 1) {
                const cv::Mat kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(erode_size, erode_size));
            ...
            const float range = std::max(white_point - black_point, 1.0e-6F);
            soft.convertTo(soft, CV_32F, 1.0F / range, -black_point / range);
            cv::threshold(soft, soft, 1.0, 1.0, cv::THRESH_TRUNC);
            cv::threshold(soft, soft, 0.0, 0.0, cv::THRESH_TOZERO);
    ...
    struct GenericOnnxProcessor::Impl {
        ...
        explicit Impl(const std::string &configuration_path) {
            ...
            if (!std::filesystem::is_regular_file(yaml_path)) {
                throw std::runtime_error("ONNX configuration is not a regular file: " + configuration_path);
            }

            const std::string yaml_text = readYamlText(yaml_path);
            cv::FileStorage storage(yaml_text, cv::FileStorage::READ | cv::FileStorage::MEMORY | cv::FileStorage::FORMAT_YAML);
            if (!storage.isOpened()) {
                throw std::runtime_error("unable to parse ONNX YAML configuration: " + configuration_path);
            }
            const cv::FileNode root = storage.root();
            validateMapKeys(root, {"model", "preprocessing", "postprocessing"}, "ONNX YAML root");

            const cv::FileNode model = root["model"];
            validateMapKeys(model, {"path", "input"}, "model");
            const std::string configured_model_path = readString(model["path"], "model.path");
            ...
            if (!std::filesystem::is_regular_file(model_path)) {
                throw std::runtime_error("ONNX model referenced by YAML is not a regular file: " + model_path.string());
            }

            const cv::FileNode preprocessing = root["preprocessing"];
            if (!preprocessing.empty()) {
                validateMapKeys(preprocessing, {"width", "height", "scale", "swap_rb", "mean", "dynamic", "alignment"}, "preprocessing");
                input_size.width = readInteger(preprocessing["width"], 224, "preprocessing.width");
                input_size.height = readInteger(preprocessing["height"], 224, "preprocessing.height");
                scale = readNumber(preprocessing["scale"], 1.0 / 255.0, "preprocessing.scale");
                swap_rb = readBoolean(preprocessing["swap_rb"], true, "preprocessing.swap_rb");
                dynamic_shape = readBoolean(preprocessing["dynamic"], false, "preprocessing.dynamic");
                shape_alignment = readInteger(preprocessing["alignment"], 4, "preprocessing.alignment");
                const cv::FileNode mean_node = preprocessing["mean"];
                if (!mean_node.empty()) {
                    if (!mean_node.isSeq() || mean_node.size() != 3U) {
                        throw std::runtime_error("preprocessing.mean must contain three numbers");
                    }
                    for (int index = 0; index < 3; ++index) {
                        mean[index] = readNumber(mean_node[index], 0.0, "preprocessing.mean");
                    }
                }
            }
            if (input_size.width < 0 || input_size.width > 16384 || input_size.height < 0 || input_size.height > 16384 || (!dynamic_shape && (input_size.width == 0 || input_size.height == 0))) {
                throw std::runtime_error("preprocessing dimensions are outside the supported range");
            }
            if (std::abs(scale) > 1.0e6 || std::any_of(&mean[0], &mean[0] + 3, [](double value) { return !std::isfinite(value) || std::abs(value) > 1.0e6; })) {
                throw std::runtime_error("preprocessing scale or mean is outside the supported range");
            }
            if (shape_alignment < 1 || shape_alignment > 1024) {
                throw std::runtime_error("preprocessing.alignment must be between 1 and 1024");
            }

            bilateral_smoothing = dynamic_shape && input_size.width > 0 && input_size.height > 0 && input_size.width <= 256 && input_size.height <= 256;
            const cv::FileNode postprocessing = root["postprocessing"];
            if (!postprocessing.empty()) {
                validateMapKeys(postprocessing, {"bilateral"}, "postprocessing");
                const cv::FileNode bilateral = postprocessing["bilateral"];
                if (!bilateral.empty() && bilateral.isMap()) {
                    validateMapKeys(bilateral, {"enabled", "diameter", "sigma_color", "sigma_space"}, "postprocessing.bilateral");
                    bilateral_smoothing = readBoolean(bilateral["enabled"], bilateral_smoothing, "postprocessing.bilateral.enabled");
                    bilateral_diameter = readInteger(bilateral["diameter"], bilateral_diameter, "postprocessing.bilateral.diameter");
                    bilateral_sigma_color = readNumber(bilateral["sigma_color"], bilateral_sigma_color, "postprocessing.bilateral.sigma_color");
                    bilateral_sigma_space = readNumber(bilateral["sigma_space"], bilateral_sigma_space, "postprocessing.bilateral.sigma_space");
                } else if (!bilateral.empty()) {
                    bilateral_smoothing = readBoolean(bilateral, bilateral_smoothing, "postprocessing.bilateral");
                }
            }
            if (bilateral_diameter < 1 || bilateral_diameter > 255 || bilateral_sigma_color < 0.0 || bilateral_sigma_color > 1.0e6 || bilateral_sigma_space < 0.0 || bilateral_sigma_space > 1.0e6) {
                throw std::runtime_error("bilateral smoothing values are outside the supported range");
            }
            ...
            net = cv::dnn::readNetFromONNX(model_path.string(), cv::dnn::ENGINE_CLASSIC);
#else
        ...
        [[nodiscard]] cv::Size resolveInputSize(const cv::Size &source_size) const {
            if (!dynamic_shape) {
                return input_size;
            }
            ...
            if (width <= 0 && height <= 0) {
                ...
                width = cvRound(height * static_cast<double>(source_size.width) / std::max(source_size.height, 1));
            } else if (height <= 0) {
                height = cvRound(width * static_cast<double>(source_size.height) / std::max(source_size.width, 1));
            }
            const auto align_dimension = [this](int value) {
                value = std::max(value, shape_alignment);
                return std::max(shape_alignment, cvRound(static_cast<double>(value) / shape_alignment) * shape_alignment);
            };
        ...
        [[nodiscard]] const cv::Mat &smoothOutput(const cv::Mat &source) {
            ...
            cv::bilateralFilter(source, smoothed, bilateral_diameter, bilateral_sigma_color, bilateral_sigma_space);
            return smoothed;
        }
        ...
        void process(const cv::Mat &image, cv::Mat &result) {
            ...
            cv::dnn::blobFromImage(image, blob, scale, frame_input_size, mean, swap_rb, false, CV_32F);
            const cv::Mat raw = selectBackendAndForward(net, backend, blob, input_name, output_name);

            if (multiple_outputs) {
                const cv::Mat plane = spatialPlane(raw);
                if (plane.empty()) {
                    throw std::runtime_error("generic ONNX multi-output result has no float plane");
                }
                cv::exp(-plane, work);
            ...
            if (raw.dims != 4 || raw.size[0] != 1 || raw.type() != CV_32F) {
                throw std::runtime_error("generic ONNX output must be a 1xCxHxW float tensor");
            }
            ...
            if (channels < 3) {
                throw std::runtime_error("generic ONNX output has fewer than three color channels");
            }
    ...
    GenericOnnxProcessor::GenericOnnxProcessor(const std::string &configuration_path) : impl(std::make_unique<Impl>(configuration_path)) {}

    GenericOnnxProcessor::~GenericOnnxProcessor() = default;

    void GenericOnnxProcessor::process(const cv::Mat &image, cv::Mat &result) { impl->process(image, result); }
    ...
    struct EdgeDetector::Impl {
        ...
        explicit Impl(const std::string &model_path) {
            if (!std::filesystem::is_regular_file(model_path)) {
                throw std::runtime_error("edge model is not a regular file: " + model_path);
            }
#if defined(CV_VERSION_MAJOR) && (CV_VERSION_MAJOR >= 5)
        ...
        void process(const cv::Mat &image, cv::Mat &result) {
            ...
            const cv::Mat raw = selectBackendAndForward(net, backend, blob, {}, output_name);
            const cv::Mat plane = spatialPlane(raw);
            if (plane.empty()) {
                throw std::runtime_error("DexiNed output does not contain a float edge plane");
            }
    ...
    EdgeDetector::EdgeDetector(const std::string &model_path) : impl(std::make_unique<Impl>(model_path)) {}

    EdgeDetector::~EdgeDetector() = default;

    void EdgeDetector::process(const cv::Mat &image, cv::Mat &result) { impl->process(image, result); }
    ...
    struct HumanSegmenter::Impl {
        ...
        explicit Impl(const std::string &model_path) {
            if (!std::filesystem::is_regular_file(model_path)) {
                throw std::runtime_error("human segmentation model is not a regular file: " + model_path);
            }
#if defined(CV_VERSION_MAJOR) && (CV_VERSION_MAJOR >= 5)
            net = cv::dnn::readNetFromONNX(model_path, cv::dnn::ENGINE_CLASSIC);
#else
        ...
        [[nodiscard]] cv::Mat infer(const cv::Mat &image) {
            ...
            cv::dnn::blobFromImage(image, blob, 1.0 / 127.5, input_size, cv::Scalar(127.5, 127.5, 127.5), false, false, CV_32F);
            const cv::Mat output = selectBackendAndForward(net, backend, blob, input_name, output_name);
            if (output.dims != 4 || output.size[0] != 1 || output.size[1] < 2 || output.type() != CV_32F) {
                throw std::runtime_error("PP-HumanSeg output is not a two-channel float mask");
            }
            ...
            cv::resize(probability, resized_mask, source_size, 0, 0, cv::INTER_CUBIC);
            if (previous_mask.empty() || previous_mask.size() != resized_mask.size()) {
                resized_mask.copyTo(previous_mask);
            } else {
                cv::addWeighted(resized_mask, 0.6, previous_mask, 0.4, 0.0, previous_mask);
            }
            return previous_mask;
        }
    };

    HumanSegmenter::HumanSegmenter(const std::string &model_path) : impl(std::make_unique<Impl>(model_path)) {}

    HumanSegmenter::~HumanSegmenter() = default;

    cv::Mat HumanSegmenter::infer(const cv::Mat &image) { return impl->infer(image); }
    ...
    cv::Mat hardenedAlphaMask(const cv::Mat &image, const cv::Mat &mask, float black_point, float white_point) {
        ...
        cv::Mat alpha;
        buildHardenedFloatAlpha(image, mask, black_point, white_point).convertTo(alpha, CV_8U, 255.0);
        return alpha;
    }
    ...
    cv::Mat isolateBody(const cv::Mat &image, const cv::Mat &mask, float black_point, float white_point) {
        ...
        const cv::Mat alpha = hardenedAlphaMask(image, mask, black_point, white_point);
        cv::Mat alpha_bgr;
