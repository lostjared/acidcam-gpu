...
int main() {
#ifdef _WIN32
    HANDLE mutex = ::CreateMutexW(nullptr, FALSE, acmxvk::ipc::SHADER_SELECTION_MUTEX_NAME_WINDOWS);
    if (mutex == nullptr) {
        std::cerr << "CreateMutexW failed: " << ::GetLastError() << '\n';
        return 1;
    }

    HANDLE mapping = ::CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, static_cast<DWORD>(sizeof(acmxvk::ipc::ShaderSelectionData)), acmxvk::ipc::SHADER_SELECTION_MAPPING_NAME_WINDOWS);
    if (mapping == nullptr) {
    ...
    void *view = ::MapViewOfFile(mapping, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(acmxvk::ipc::ShaderSelectionData));
    if (view == nullptr) {
    ...
    auto *selection = static_cast<acmxvk::ipc::ShaderSelectionData *>(view);
    {
