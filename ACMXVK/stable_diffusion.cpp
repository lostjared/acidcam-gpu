...
namespace acmxvk::stable_diffusion {
    namespace {
        ...
        std::size_t appendResponse(char *data, std::size_t size, std::size_t count, void *context) {
            ...
            if (bytes > MAX_RESPONSE_SIZE - std::min(buffer->value.size(), MAX_RESPONSE_SIZE)) {
                buffer->overflow = true;
                return 0U;
            }
        ...
        int checkCancelled(void *context, curl_off_t, curl_off_t, curl_off_t, curl_off_t) {
            ...
            return cancelled != nullptr && *cancelled && (*cancelled)() ? 1 : 0;
        }
        ...
        [[nodiscard]] std::vector<std::uint8_t> decodeBase64(std::string_view encoded) {
            ...
            for (const unsigned char character : encoded) {
                ...
                if (value < 0) {
                    ...
                    throw std::runtime_error("sd-server returned invalid base64 image data");
                }
                accumulator = (accumulator << 6U) | static_cast<std::uint32_t>(value);
                bits += 6;
                if (bits >= 8) {
                    bits -= 8;
                    output.push_back(static_cast<std::uint8_t>((accumulator >> static_cast<unsigned int>(bits)) & 0xffU));
                }
            }
            return output;
        }

        [[nodiscard]] Json::Value parseJson(std::string_view text, std::string_view context) {
            ...
            if (!reader->parse(text.data(), text.data() + text.size(), &value, &error)) {
                throw std::runtime_error(std::string(context) + " returned invalid JSON: " + error);
            }
            return value;
        }
        ...
        [[nodiscard]] ResponseBuffer request(std::string_view url, const std::string *body, long timeout_seconds, long &status, const std::function<bool()> *cancelled) {
            ...
            if (result != CURLE_OK) {
                ...
                if (response.overflow) {
                    throw std::runtime_error("sd-server response exceeded 64 MiB");
                }
                throw std::runtime_error("sd-server request failed: " + curlError(result));
            }
            return response;
        }
        ...
        [[nodiscard]] cv::Size neuralUpscaleWorkingSize(const Settings &settings, const cv::Size &fallback) {
            ...
            if (pixels > MAX_WORKING_PIXELS) {
                ...
                height = std::max(64, static_cast<int>(std::floor(height * scale)));
            }
            return {width, height};
        }
    ...
    void Server::start() {
        ...
        if (settings.quiet) {
            std::string log_template = (std::filesystem::temp_directory_path() / "acmxvk-sd-server-XXXXXX").string();
            std::vector<char> mutable_template(log_template.begin(), log_template.end());
            mutable_template.push_back('\0');
            const int log_fd = ::mkstemp(mutable_template.data());
            if (log_fd < 0) {
                throw std::runtime_error("unable to create sd-server diagnostic log: " + std::string(std::strerror(errno)));
            }
            ::close(log_fd);
            diagnostic_log_path = mutable_template.data();

            const int init_result = posix_spawn_file_actions_init(&file_actions);
            if (init_result != 0) {
                throw std::runtime_error("unable to configure sd-server output: " + std::string(std::strerror(init_result)));
            }
            actions = &file_actions;
            const int stdout_result = posix_spawn_file_actions_addopen(actions, STDOUT_FILENO, diagnostic_log_path.c_str(), O_WRONLY | O_APPEND, 0600);
            const int stderr_result = posix_spawn_file_actions_adddup2(actions, STDOUT_FILENO, STDERR_FILENO);
            if (stdout_result != 0 || stderr_result != 0) {
                posix_spawn_file_actions_destroy(actions);
                const int error = stdout_result != 0 ? stdout_result : stderr_result;
                throw std::runtime_error("unable to redirect sd-server output: " + std::string(std::strerror(error)));
            }
        }
        const int result = posix_spawnp(&child, executable.c_str(), actions, nullptr, arguments.data(), environ);
        if (actions != nullptr) {
            posix_spawn_file_actions_destroy(actions);
        }
        if (result != 0) {
    ...
    std::string Server::diagnosticLogDetails() const {
        ...
        std::ifstream input(diagnostic_log_path, std::ios::binary | std::ios::ate);
        std::string details = "; sd-server diagnostic log: " + diagnostic_log_path.string();
        if (!input) {
            return details;
        }
        ...
        const std::streamoff start = std::max<std::streamoff>(0, size - MAX_LOG_TAIL);
        input.seekg(start);
    ...
    void Server::stop() noexcept {
        ...
        if (kill(child, SIGTERM) != 0 && errno != ESRCH) {
            std::cerr << "acmxvk: unable to stop sd-server process " << process_id << ": " << std::strerror(errno) << '\n';
        }
    ...
    void Server::waitUntilReady() {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            ...
            long status = 0;
            try {
                const ResponseBuffer response = request(url, nullptr, 3L, status, &settings.cancelled);
                if (status == 200) {
                    if (!response.value.empty()) {
                        const Json::Value document = parseJson(response.value, "sd-server");
                        if (!settings.upscale_model.empty()) {
                            const std::string upscaler_name = settings.upscale_model.stem().string();
                            bool found = false;
                            for (const Json::Value &entry : document["upscalers"]) {
                                if (entry["name"].asString() == upscaler_name) {
                                    found = true;
                                    break;
                                }
                            }
                            if (!found) {
                                throw std::runtime_error("sd-server did not discover the requested "
                                                         "upscaler model: " +
                                                         upscaler_name);
                            }
                        }
                        for (const Lora &lora : settings.loras) {
                            bool found = false;
                            for (const Json::Value &entry : document["loras"]) {
                                if (entry["path"].asString() == lora.path.generic_string()) {
                                    found = true;
                                    break;
                                }
                            }
                    ...
                    if (!settings.loras.empty()) {
                        std::cout << "; " << settings.loras.size() << " LoRA model(s)";
                    }
                    std::cout << '\n';
                    resetDiagnosticLog();
                    return;
        ...
        throw std::runtime_error("sd-server did not become ready within five minutes" + diagnosticLogDetails());
    }

    cv::Mat Server::process(const cv::Mat &rgba) const {
        if (rgba.empty() || rgba.type() != CV_8UC4) {
            throw std::runtime_error("Stable Diffusion input must be a non-empty RGBA8 frame");
        }
        cv::Mat bgr;
        cv::cvtColor(rgba, bgr, cv::COLOR_RGBA2BGR);
        std::vector<std::uint8_t> png;
        if (!cv::imencode(".png", bgr, png)) {
            throw std::runtime_error("unable to encode Stable Diffusion input frame");
        }
        ...
        if (settings.upscale_model.empty()) {
            ...
            long status = 0;
            const ResponseBuffer response = request(endpoint + "/sdapi/v1/img2img", &body, 3600L, status, &settings.cancelled);
            const Json::Value document = parseJson(response.value, "sd-server");
            if (status != 200) {
                throw std::runtime_error("sd-server rejected frame: " + serverError(document));
            }
            if (!document["images"].isArray() || document["images"].empty() || !document["images"][0].isString()) {
                throw std::runtime_error("sd-server response did not contain an output image");
            }
        ...
        if (output_bgr.empty()) {
            throw std::runtime_error("unable to decode image returned by sd-server");
        }
        cv::Mat output_rgba;
        cv::cvtColor(output_bgr, output_rgba, cv::COLOR_BGR2RGBA);
        if (!settings.upscale_model.empty() && settings.upscale_width > 0 && settings.upscale_height > 0 && output_rgba.size() != cv::Size(settings.upscale_width, settings.upscale_height)) {
            cv::resize(output_rgba, output_rgba, {settings.upscale_width, settings.upscale_height}, 0.0, 0.0, cv::INTER_LANCZOS4);
        } else if (settings.resize_to_input && output_rgba.size() != rgba.size()) {
            cv::resize(output_rgba, output_rgba, rgba.size(), 0.0, 0.0, cv::INTER_LINEAR);
        }
        return output_rgba;
    }
} // namespace acmxvk::stable_diffusion
