...
void updateAndDraw(cv::Mat &frame, ac_gpu::DynamicFrameBuffer &buffer,
    ...
    if (gState.alpha_dir == 1) {
        gState.alpha += 0.01f;
        if (gState.alpha >= 3.0f)
            gState.alpha_dir = 0;
    } else {
        gState.alpha -= 0.01f;
        if (gState.alpha <= 1.0f)
            gState.alpha_dir = 1;
    }
...
int main(int argc, char **argv) {
    ...
    if (list.find(',') != std::string::npos) {
        ...
        while (1) {
            ...
            if (!tok.empty())
                vlist.emplace_back(ac_gpu::Filter{std::stoi(tok), ac_gpu::filters[std::stoi(tok)].name});
    ...
    try {
        ...
        while (1) {
            ...
            if (camera_mode == false && tick_count == 1) {
                ...
                if (elapsed.count() < frame_duration.count())
                    std::this_thread::sleep_for(std::chrono::milliseconds(frame_duration.count() - elapsed.count()));
            }
        }
